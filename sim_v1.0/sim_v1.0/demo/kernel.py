...
class kernel(object):
    def __init__(self, car_num, time, render=False):
        ...
        self.time = time
        self.render = render
        # below are params that can be changed depended on situation
        self.bullet_speed = 12.5  # 子弹速度，单位为pixel
        self.motion = 6  # 移动的惯性感大小 ori = 6
        self.rotate_motion = 6  # 底盘旋转的惯性感大小 ori = 6
        self.yaw_motion = 1  # 云台旋转的惯性感大小 ori = 1
        self.camera_angle = 75 / 2  # 摄像头的视野范围
        self.move_discount = 0.6  # 撞墙之后反弹的强度大小
        # above are params that can be changed depended on situation
        self.map_length = 800
        self.map_width = 600
        ...
        if render:
            ...
            for oi, o in enumerate(['red', 'blue']):
                for ti in range(5):  # enumerate(['bonus', 'supply', 'start', 'start', 'start']):
        ...
        while True:
            ...
            if not self.epoch % 10:
                if self.get_order():
                    endFlag.put(1)
                    break
                # print('car_info: [team, x, y, yaw, yaw_PTZ, heat, HP]')
                # print(self.cars[:, 0:7], '\n')
            self.one_epoch(cars_info, guide_info)
            if self.time <= 0 or (self.cars[0:5, 6] == np.zeros(5)).all() or (self.cars[5:, 6] == np.zeros(5)).all():
                if self.cars[0:5, 6].sum() == self.cars[5:, 6].sum():
                    return -1
                else:
                    return int(self.cars[0:5, 6].sum() < self.cars[5:, 6].sum())
    ...
    def one_epoch(self):
        ...
        for n in range(self.car_num):  # 循环更改车辆参数
            ...
            self.move_car(n)
            '''
            if not self.epoch % 20: # 功率超标扣血，////可删除
    ...
    def dev_window(self):
        ...
        for n in range(self.car_num):  # 显示车辆详细信息
        ...
        info = self.font.render(
            'red   supply: {}   bonus: {}   bonus_time: {}'.format(self.compet_info[0, 0], self.compet_info[0, 1],
                                                                   self.compet_info[0, 3]), False, (0, 0, 0))
        self.screen.blit(info, (8, 372))
        info = self.font.render(
            'blue   supply: {}   bonus: {}   bonus_time: {}'.format(self.compet_info[1, 0], self.compet_info[1, 1],
                                                                    self.compet_info[1, 3]), False, (0, 0, 0))
            ...
            for i in range(self.car_num - 1):
                ...
                if abs(angle) < self.camera_angle:
                    if self.line_barriers_check(self.cars[n, 1:3], self.cars[n - i - 1, 1:3]) \
                            or self.line_cars_check(self.cars[n, 1:3], self.cars[n - i - 1, 1:3]):  # 往左遍历
